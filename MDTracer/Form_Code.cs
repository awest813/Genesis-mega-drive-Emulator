    ...
    public partial class Form_Code : Form
    {
        ...
        private void pictureBox_code_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                int w_jmp_line = md_main.g_form_code_trace.get_code_from_addr((uint)w_jmp);
                if ((w_jmp != 0) && (w_jmp_line >= 0))
                {
                    md_main.g_form_code_trace.g_arrow_start_line = w_cur;
                    md_main.g_form_code_trace.g_arrow_end_line = w_jmp_line;
                }
                else
                {
                    md_main.g_form_code_trace.g_arrow_start_line = -1;
        ...
        private int GetNextCodeLine(int in_line)
        {
            ...
            int w_line = in_line + Math.Max(1, md_main.g_form_code_trace.g_analyse_code[in_line].leng2);
        ...
        private int GetPreviousCodeLine(int in_line)
        {
            ...
            int w_front = Math.Max(0, md_main.g_form_code_trace.g_analyse_code[w_line].front);
        ...
        private int NormalizeCodeLine(int in_line)
        {
            ...
            int w_front = Math.Max(0, md_main.g_form_code_trace.g_analyse_code[in_line].front);
        ...
        private void ToggleBreakpoint(int in_line)
        {
            ...
            TRACECODE w_code = md_main.g_form_code_trace.g_analyse_code[in_line];
            w_code.break_static = w_code.break_static == false;
            md_main.g_form_code_trace.g_analyse_code[in_line] = w_code;
        ...
        private string GetBreakpointSettingText()
        {
            ...
            {
                TRACECODE w_code = md_main.g_form_code_trace.g_analyse_code[i];
        ...
        private void SetBreakpointSettingText(string in_text)
        {
            for (int i = 0; i < Form_Code_Trace.MEMSIZE; i++)
            {
                ...
                md_main.g_form_code_trace.g_analyse_code[i] = w_code;
            }
            ...
            foreach (string w_address_text in SplitGameSettingEntries(in_text))
            {
                ...
                md_main.g_form_code_trace.g_analyse_code[w_line] = w_code;
            }
        }
        ...
        private void WriteTraceCodeCsv(string in_path)
        {
            using (StreamWriter w_writer = new StreamWriter(in_path, false, new UTF8Encoding(true)))
            {
                ...
                {
                    TRACECODE w_code = md_main.g_form_code_trace.g_analyse_code[w_cur_line];
        ...
        private int GetTraceCodeDumpLength(int in_line)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                if (md_main.g_form_code_trace.g_analyse_code[in_line + i].type != TRACECODE.TYPE.NON) break;
                w_dump_len += 1;
            }
            return w_dump_len;
        }
        private string GetTraceCodeDumpText(int in_line, int in_dump_len)
        {
            ...
            for (int i = 0; i < in_dump_len; i++)
            {
                ...
                w_builder.Append(md_main.g_form_code_trace.g_analyse_code[in_line + i].val.ToString("X4"));
            }
        ...
        private void textBoxAddr_TextChanged_1(object sender, EventArgs e)
        {
            ...
            {
                int w_line = md_main.g_form_code_trace.get_code_from_addr(waddr);
                if (w_line >= 0)
                {
        ...
        private bool TryGetSelectedCommentSearchLine(out int out_line)
        {
            ...
            {
                out_line = md_main.g_form_code_trace.get_code_from_addr(w_addr);
                return out_line >= 0;
            }
        ...
        private void dataGridView_memory_history_CellDoubleClick(object? sender, DataGridViewCellEventArgs e)
        {
            ...
            RefreshComment1List(true);
            int w_line = md_main.g_form_code_trace.get_code_from_addr(w_pc);
            if (w_line < 0) return;
