namespace MDTracer
{
    ...
    internal sealed class WinFormsMainLoopUiHooks : IMainLoopUiHooks
    {
        ...
        public void LoadCurrentGameCodeSettings()
        {
            md_main.g_form_code.LoadCurrentGameCodeSettings();
        }

        public void UpdateCodeTrace()
        {
            md_main.g_form_code_trace.update();
        }

        public void PushTopTraceEntry(uint in_pc, uint in_stackAddress)
        {
            md_main.g_form_code_trace.CPU_Trace_push(M68kStackEntryType.TOP, 0x0004, in_pc, 0, in_stackAddress);
        }

        public void TraceFirstStepBreak()
        {
            md_main.g_form_code_trace.Trace_FirstStepBreak();
        }
        ...
        public void UpdateDebugWindows()
        {
            ...
            {
                md_main.g_form_screenA.picture_update(md_main.g_md_vdp.g_scrollA_bitmap
                                                        , md_main.g_md_vdp.g_scroll_xsize
                                                        , md_main.g_md_vdp.g_scroll_ysize);
            }
            if (md_main.g_screenB_enable == true)
            {
                md_main.g_form_screenB.picture_update(md_main.g_md_vdp.g_scrollB_bitmap
                                                        , md_main.g_md_vdp.g_scroll_xsize
                                                        , md_main.g_md_vdp.g_scroll_ysize);
            }
            if (md_main.g_screenW_enable == true)
            {
                md_main.g_form_screenW.picture_update(md_main.g_md_vdp.g_scrollW_bitmap
                                                        , md_main.g_md_vdp.g_scroll_xsize
                                                        , md_main.g_md_vdp.g_scroll_ysize);
            }
            if (md_main.g_screenS_enable == true)
            {
                md_main.g_form_screenS.picture_update(md_main.g_md_vdp.g_scrollS_bitmap
                                                        , md_main.g_md_vdp.SPRITE_XSIZE
                                                        , md_main.g_md_vdp.SPRITE_YSIZE);
            }
            if (md_main.g_pattern_enable == true)
            {
                md_main.g_form_pattern.picture_update(md_main.g_md_vdp.g_pattern_table);
            }
            if (md_main.g_pallete_enable == true)
            {
                md_main.g_form_pallete.Invalidate();
            }
            if (md_main.g_music_enable == true)
            {
                for (int i = 0; i < 10; i++)
                {
                    md_main.g_form_music.g_freq_out[i] = md_main.g_md_music.g_freq_out[i];
                }
                md_main.g_form_music.Invalidate();
            }
        ...
        public void FlashTraceBreakAtAddress(uint in_pc)
        {
            int w_line = md_main.g_form_code_trace.get_code_from_addr(in_pc);
            if (w_line >= 0)
            {
                md_main.g_form_code_trace.g_analyse_code[w_line].break_flash = true;
            }
        }

        public void ResetCodeTraceAnalysis()
        {
            md_main.g_form_code_trace.analyses_reset();
        }
    }
}
